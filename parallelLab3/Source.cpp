...
void laba(int argc, char **argv)
{
	...
	if (rank == 0)
	{
		cout << changeStrCode("–азмер массива ") << N << endl;
		cout << changeStrCode("ћассив последовательно отсортирован") << endl;

		if (arrSorted(arr, N))
			cout << changeStrCode("ћассив отсортирован параллельно") << endl;
		else
			cout << "WTF?";
		cout << endl;
		
		cout << changeStrCode("ѕоследовательное врем€ ") << no_par_time << changeStrCode(" мс") << endl;
		cout << changeStrCode("ѕараллельное врем€ ") << par_time << changeStrCode(" мс") << endl;
		if(arrCmp(arr, arr2, N))
		cout << changeStrCode("—равнение массивов- массивы одинаковы")  << endl;
		else
			cout << changeStrCode("ћассивы различны") << endl;
